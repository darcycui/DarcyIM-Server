...
//        val accessor = StompHeaderAccessor.wrap(message)
        ...
        accessor.run {
            logI("$TAG preSend command=${command?.name}")
            when (command) {
                ...
                StompCommand.SUBSCRIBE -> {
                    ...
                    logI("$TAG 用户$userId 订阅了 $destination")
                }

                StompCommand.DISCONNECT -> {
                    val userName = user?.name ?: ""
                    logI("$TAG 用户$userName 下线了")
                    // 更新用户的最后活跃时间（即离线时间）
...
//                    TransportKeyManager.removeTransportKey(userId)
                ...
                StompCommand.UNSUBSCRIBE -> {
                    val userId = user?.name ?: ""
                    logI("$TAG 用户$userId 取消订阅了 $destination")
                }

                StompCommand.SEND -> {
                    val userId = user?.name ?: ""
                    logI("$TAG 用户$userId 发送了消息到 $destination")
                }
                ...
                StompCommand.RECEIPT -> {
                    val userId = user?.name ?: ""
                    logI("$TAG 用户$userId 接收了 RECEIPT 确认帧 $receiptId")
                }

                StompCommand.MESSAGE -> {}
                StompCommand.ERROR -> {
                    val userId = user?.name ?: ""
                    logI("$TAG 用户$userId 接收到错误帧 $sessionId")
                }
    ...
    override fun afterSendCompletion(message: Message<*>, channel: MessageChannel, sent: Boolean, ex: Exception?) {
        ...
        if (ex != null) {
            val accessor = StompHeaderAccessor.wrap(message)
            logE("$TAG afterSendCompletion 异常: ${ex.message}" +
    ...
    private fun registerUserName(accessor: StompHeaderAccessor) {
        ...
        logI("$TAG 从STOMP头获取 userName: $userName")
    ...
    private fun setupUserNameForSTOMP(accessor: StompHeaderAccessor, userName: String) {
        ...
        if (verifyUserId(userName).not()) {
            logE("$TAG 用户 userName：$userName 不合法")
        }
...
//        return userName.startsWith("test")
    ...
    override fun onApplicationEvent(event: SessionConnectedEvent) {
        ...
        logI("$TAG SessionConnectedEvent: 用户 $user 已完成连接，sessionId: $sessionId")
//        userCount()
    }
}